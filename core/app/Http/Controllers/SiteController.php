...
{
    $article = Articlee::findOrFail($id);
    return view('knowledge', compact('article'));
}
    ...
    {
        if($id != null ){
            $art = Articlee::where('id',$id)->first();
        }else{
            $articlesQuery = Articlee::searchable(['name', 'description', 'course_category:name'])
                ->with(['course_category'])
                ->orderBy('id', 'DESC')->first();
            if ($articlesQuery) {
                $art = $articlesQuery ;
            } else {
                $art = Articlee::where('id',84)->first();
            }
        }
        $categories = CourseCategory::with('articles')->get();
        $pageTitle = 'Knowledge';

        return view('knowledgebase.knowledge', ['categories' => $categories ,'pageTitle'=> $pageTitle ,'art'=>$art]);
...
//     // Récupérer tous les articles ou les articles filtrés par terme de recherche
//     $articlesQuery = Articlee::searchable(['name', 'description', 'course_category:name'])
//         ->with(['course_category'])
//         ->orderBy('id', 'DESC');
//         $articles = $articlesQuery->paginate(getPaginate());

//     // Définir le titre de la page
//     $pageTitle = 'Articles';

//     // Définir la variable $sections si nécessaire
//     $sections = Page::where('tempname', $this->activeTemplate)->where('slug', 'articles')->first();

//     return view($this->activeTemplate . 'articles', compact('pageTitle', 'articles', 'sections'));
// }
